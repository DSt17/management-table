    ...
    methods: {
	  ...
	  onSubmit() {
		if (this.newUserName.trim() !== '' && this.taskTitle.trim() !== ''
		    ...
		    const newUserFromCompany = {
			  task: this.taskTitle,
			  customer: this.newUserName,
			  workingTimeHours: 5,
			  dayStart: this.dayStart.replace(/-/g, '/'),
			  dayFinish: this.dayFinish.replace(/-/g, '/')
		    }
		    
		    this.$emit('AddNewUserInACompany', newUserInAState, newUserFromCompany, this.selectedCustomer)
		    this.newUserName = ''
